...
// category fetching is now removed:
        ...
        try {
            if (!empty($_POST['event_id']) && ctype_digit((string)$_POST['event_id'])) {
                ...
                if ($posterFilename !== null) {
                    // Category_ID removed from UPDATE query
                    $stmt = $pdo->prepare('UPDATE event SET Date = ?, Time = ?, Venue = ?, Name = ?, poster = ? WHERE Event_ID = ?');
                    $stmt->execute([$date, $time ?: null, $venue, $name, $posterFilename, $eventId]);
                } else {
                    // Category_ID removed from UPDATE query
                    $stmt = $pdo->prepare('UPDATE event SET Date = ?, Time = ?, Venue = ?, Name = ? WHERE Event_ID = ?');
                    $stmt->execute([$date, $time ?: null, $venue, $name, $eventId]);
                }
                ...
                // insert new
                // Category_ID removed from INSERT query
                $stmt = $pdo->prepare('INSERT INTO event (Date, Time, Venue, Name, poster) VALUES (?, ?, ?, ?, ?)');
                $stmt->execute([$date, $time ?: null, $venue, $name, $posterFilename]);
    ...
    <body class="other-page">
        ...
        <div class="eventBox">
            ...
            <form action="admin_ConcertAdd.php<?= $editing ? '?id=' . (int)$editingEvent['Event_ID'] : '' ?>" method="post" enctype="multipart/form-data">
                ...
                <?php
                // Category input/hidden field removed
                /*
                if (!empty($editingEvent) && !empty($editingEvent['Category_ID'])):
                ?>
                  <input type="hidden" name="category_id" value="<?= (int)$editingEvent['Category_ID'] ?>">
                <?php endif;
                */
                ?>
